 ...
 */
export const mockFetchJsonError = (status: number, problemDetails: Record<string, any>) => {
  return vi.fn().mockResolvedValue(
 ...
 */
export const mockFetchByUrl = (responses: Record<string, { status: number; body: any }>) => {
  return vi.fn((url: string) => {
    const matched = Object.entries(responses).find(([pattern]) => new RegExp(pattern).test(url));
 ...
 */
export const expectFetchCalledWith = (url: string | RegExp, options?: Partial<RequestInit>) => {
